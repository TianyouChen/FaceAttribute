...
def train(epoch, model, loss_fn, optimizer, dataloader,log_interval=50):
    ...
    for i_batch, sample_batched in enumerate(dataloader):
        ...
        #train_loss += loss.data.cpu().numpy()[0] ##>pytorch 0.3 do not need [0]
        if i_batch % log_interval == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\t\tLoss: {:.6f}'.format(
                epoch, i_batch , len(dataloader),
                100. * i_batch / len(dataloader), loss.data))
            line = "Train Epoch: " + str(epoch) + " " + str(100. * i_batch / len(dataloader)) + " " + str(loss.data)
            add(line)
...
for epoch in range(start_epoch, end_epoch + 1):
    train_loss = train(epoch, model, loss, optimizer, dataLoaderTrain,  log_interval=1)

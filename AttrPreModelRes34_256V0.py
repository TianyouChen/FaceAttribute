...
class FeatureExtraction(torch.nn.Module):
    def __init__(self):
        ...
        #self.resnet = models.alexnet(pretrained=False)
...
class Classifier(nn.Module):
    def __init__(self, output_dim=1):
        ...
        self.fc1 = nn.Sequential(
            nn.Linear(512, 512),  ##resnet34 the last second layer output is 512 ,so the fc1 nn.Linear(input_channel, 512) must be nn.Linear(512, 512)
            nn.ReLU(True),
            nn.Dropout(p=0.5),
            nn.Linear(512, 128),
            nn.ReLU(True),
